...
public class ObjectPool : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            //change enemyprefab randomize the list 
            Debug.Log("the count "+ enemyprefablist.Count);
            enemyprefab = enemyprefablist[UnityEngine.Random.Range(0, enemyprefablist.Count)];
            GameObject enemy = Instantiate(enemyprefab, Vector3.zero, Quaternion.identity, gameObject.transform);
            enemylist.Add(enemy);
            enemylist[i].transform.name = enemylist[i].transform.GetSiblingIndex().ToString();
